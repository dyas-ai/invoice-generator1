...
def preprocess_excel_flexible_auto(uploaded_file, max_rows=20):
    ...
    # Enhanced filtering to remove template/dummy data
    df = df[~df["STYLE NO"].isin(["", "nan", "NaN", "None", "NONE"])]
    df = df[~df["STYLE NO"].str.contains("total|grand|remarks|note", case=False, na=False)]
    
    # Filter out common template style codes and patterns
    template_patterns = [
        "SA0167A21",  # Specific template code to exclude
        "SAMPLE",
        "TEMPLATE", 
        "EXAMPLE",
        "TEST",
    ...
    for pattern in template_patterns:
        df = df[~df["STYLE NO"].str.contains(pattern, case=False, na=False)]
    
    # Additional filtering: remove rows where STYLE NO looks like a template pattern
    df = df[~df["STYLE NO"].str.match(r'^[A-Z]{2}\d{4}[A-Z]\d{2}
...
if uploaded_file is not None:
    try:
        ...
        st.error(f"❌ Error: {e}"))]  # Pattern like SA0167A21
    
    # Filter out rows with zero or very low quantities that might be templates
    df = df[~((df["QTY"].astype(float) <= 0) & (df["STYLE NO"].str.len() > 5))]
