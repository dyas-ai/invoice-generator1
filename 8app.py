...
def preprocess_excel_flexible_auto(uploaded_file, max_rows=20):
    # First read with openpyxl to detect hidden rows
    from openpyxl import load_workbook
    
    # Load workbook to check for hidden rows
    ...
    for row_num in range(1, worksheet.max_row + 1):
        row = worksheet.row_dimensions[row_num]
        if not getattr(row, 'hidden', False):  # Include only non-hidden rows
            visible_rows.append(row_num - 1)  # Convert to 0-based indexing
    
    # Now read the Excel file with pandas
    df_raw = pd.read_excel(uploaded_file, header=None)
    
    # Filter to only visible rows
    df_raw = df_raw.iloc[visible_rows].reset_index(drop=True)
    ...
    # Read fabric type from column N, row 5 of visible rows only
    try:
