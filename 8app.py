...
def generate_proforma_invoice(df, form_data):
    ...
    # Supplier & Consignee sections (unchanged)
    # ...

    # Goods description WITHOUT line
    goods_data = [[Paragraph(f"<b>Description of goods:</b> {form_data['goods_desc']}", 
                              ParagraphStyle('Goods', parent=normal_style, fontSize=7)), ""]]
    goods_table = Table(goods_data, colWidths=[7.3*inch, 0],
                        style=[('BOX',(0,0),(-1,-1),0,colors.white),
                               ('VALIGN',(0,0),(-1,-1),'MIDDLE')])
    goods_table._argH[0] = 25
    ...
    currency_table = Table(currency_data, colWidths=[7.3*0.75*inch, 7.3*0.25*inch],
                           style=[('BOX',(0,0),(-1,-1),1,colors.black),
                                  ('VALIGN',(0,0),(-1,-1),'MIDDLE')])
    currency_table._argH[0] = 25
    ...
    product_table = Table(table_data, colWidths=[0.8*inch,1.3*inch,0.8*inch,0.7*inch,1.1*inch,0.7*inch,0.5*inch,0.6*inch,0.8*inch], repeatRows=1)
    product_table.setStyle(TableStyle([
        ...
        ('LINEBEFORE',(1,0),(8,-1),0.5,colors.black),
    ...
    signature_table = Table(signature_data, colWidths=[3.65*inch,3.65*inch],
                            style=[('BOX',(0,0),(-1,-1),1,colors.black),
...
if uploaded_file is not None:
    try:
        ...
        if st.button("Generate PDF"):
            form_data = {
                "pi_number": pi_number, "order_ref": order_ref, "buyer_name": buyer_name,
                "brand_name": brand_name, "consignee_name": consignee_name, "consignee_address": consignee_address,
                "consignee_tel": consignee_tel, "payment_term": payment_term, "bank_beneficiary": bank_beneficiary,
                "bank_account": bank_account, "bank_name": bank_name, "bank_address": bank_address,
                "bank_swift": bank_swift, "bank_code": bank_code, "loading_country": loading_country,
                "port_loading": port_loading, "shipment_date": shipment_date, "remarks": remarks,
